...
func SetupUnbondingTests(
	...
	hookCalled *bool,
	ubdeID *uint64,
) (bondDenom string, addrDels []sdk.AccAddress, addrVals []sdk.ValAddress) {
	// setup hooks
	...
	mockStackingHooks.EXPECT().AfterUnbondingInitiated(gomock.Any(), gomock.Any()).DoAndReturn(func(ctx sdk.Context, id uint64) error {
		...
		*ubdeID = id
		...
		err := stakingKeeper.PutUnbondingOnHold(ctx, id)
	...
	stakingKeeper.SetHooks(types.NewMultiStakingHooks(mockStackingHooks))

	addrDels = simtestutil.AddTestAddrsIncremental(bankKeeper, stakingKeeper, ctx, 2, math.NewInt(10000))
	addrVals = simtestutil.ConvertAddrsToValAddrs(addrDels)

	valTokens := stakingKeeper.TokensFromConsensusPower(ctx, 10)
	startTokens := stakingKeeper.TokensFromConsensusPower(ctx, 20)
	...
	bankKeeper.SendCoinsFromModuleToModule(ctx, types.BondedPoolName, types.NotBondedPoolName, sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, startTokens)))
	...
	validator1 = stakingkeeper.TestingUpdateValidator(stakingKeeper, ctx, validator1, true)
	...
	validator2 = stakingkeeper.TestingUpdateValidator(stakingKeeper, ctx, validator2, true)
...
func TestValidatorUnbondingOnHold1(t *testing.T) {
	...
	_, _, addrVals := SetupUnbondingTests(t, stakingKeeper, bankKeeper, accountKeeper, ctx, &hookCalled, &ubdeID)

	// Start unbonding first validator
	validator := doValidatorUnbonding(t, stakingKeeper, ctx, addrVals[0], &hookCalled)
	...
	err := stakingKeeper.UnbondingCanComplete(ctx, ubdeID)
	require.NoError(t, err)

	// Try to unbond validator
	stakingKeeper.UnbondAllMatureValidators(ctx)
	...
	unbondingVals := stakingKeeper.GetUnbondingValidators(ctx, completionTime, completionHeight)
...
func TestValidatorUnbondingOnHold2(t *testing.T) {
	...
	// Start unbonding first validator
	validator1 := doValidatorUnbonding(t, stakingKeeper, ctx, addrVals[0], &hookCalled)
	...
	validator2 := doValidatorUnbonding(t, stakingKeeper, ctx, addrVals[1], &hookCalled)
	ubdeIDs = append(ubdeIDs, ubdeID)
	...
	validator1, found := stakingKeeper.GetValidator(ctx, addrVals[0])
	require.True(t, found)
	require.Equal(t, types.Unbonding, validator1.Status)
	validator2, found = stakingKeeper.GetValidator(ctx, addrVals[1])
	require.True(t, found)
	...
	// CONSUMER CHAIN'S UNBONDING PERIOD ENDS - STOPPED UNBONDING CAN NOW COMPLETE
	err := stakingKeeper.UnbondingCanComplete(ctx, ubdeIDs[0])
	require.NoError(t, err)
	...
	err = stakingKeeper.UnbondingCanComplete(ctx, ubdeIDs[1])
	require.NoError(t, err)
...
func TestRedelegationOnHold1(t *testing.T) {
	...
	stakingKeeper, bankKeeper, accountKeeper, ctx := createTestInput(t)
	_, addrDels, addrVals := SetupUnbondingTests(t, stakingKeeper, bankKeeper, accountKeeper, ctx, &hookCalled, &ubdeID)
	completionTime := doRedelegation(t, stakingKeeper, ctx, addrDels, addrVals, &hookCalled)
	...
	_, err = stakingKeeper.CompleteRedelegation(ctx, addrDels[0], addrVals[0], addrVals[1])
	require.NoError(t, err)
...
func TestUnbondingDelegationOnHold1(t *testing.T) {
	...
	stakingKeeper, bankKeeper, accountKeeper, ctx := createTestInput(t)
	bondDenom, addrDels, addrVals := SetupUnbondingTests(t, stakingKeeper, bankKeeper, accountKeeper, ctx, &hookCalled, &ubdeID)
	completionTime, bondedAmt1, notBondedAmt1 := doUnbondingDelegation(t, stakingKeeper, bankKeeper, ctx, bondDenom, addrDels, addrVals, &hookCalled)

	// CONSUMER CHAIN'S UNBONDING PERIOD ENDS - BUT UNBONDING CANNOT COMPLETE
	err := stakingKeeper.UnbondingCanComplete(ctx, ubdeID)
	require.NoError(t, err)

	bondedAmt3 := bankKeeper.GetBalance(ctx, stakingKeeper.GetBondedPool(ctx).GetAddress(), bondDenom).Amount
	notBondedAmt3 := bankKeeper.GetBalance(ctx, stakingKeeper.GetNotBondedPool(ctx).GetAddress(), bondDenom).Amount
	...
	bondedAmt5 := bankKeeper.GetBalance(ctx, stakingKeeper.GetBondedPool(ctx).GetAddress(), bondDenom).Amount
	notBondedAmt5 := bankKeeper.GetBalance(ctx, stakingKeeper.GetNotBondedPool(ctx).GetAddress(), bondDenom).Amount
